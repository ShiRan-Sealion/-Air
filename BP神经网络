%此版本在源代码基础上做了一定的修改,已经完成了输出
%%
%输入样本数量
numberOfSample = 20; 
%取测试样本数量等于输入(训练集)样本数量，因为输入样本（训练集）容量较少，否则一般必须用新鲜数据进行测试
numberOfTestSample = 20; 
numberOfForcastSample = 2; %预测样本数
numberOfHiddenNeure = 8; %隐含层神经元个数
inputDimension = 3; %输入数据矩阵的维度
outputDimension = 2; %输出数据矩阵的维度
...
%输入数据矩阵
input = [numberOfPeople; numberOfAutomobile; roadArea]; %对应inputDimension
%目标（输出）数据矩阵
output = [passengerVolume; freightVolume]; %对应outputDimension

%对训练集中的输入数据矩阵和目标数据矩阵进行归一化处理
[sampleInput, minp, maxp, tmp, mint, maxt] = premnmx(input, output);
%对input和output进行归一化处理，结果为sampleInput、tmp
%minp、maxp是input每行最值;mint、maxt是output最值
...
%%
%测试集
%取测试样本输入(输出)与输入样本相同，因为输入样本（训练集）容量较少，否则一般必须用新鲜数据进行测试（ANN中加入了测试数据）
...
legend('神经网络输出客运量', '实际客运量');
xlabel('年份'); ylabel('客运量/万人');
title('神经网络客运量训练与测试对比图');
grid on;

subplot(2, 1, 2); plot(t, a2, 'ro', t, freightVolume, 'b+');
legend('神经网络输出货运量', '实际货运量');
xlabel('年份'); ylabel('货运量/万吨');
title('神经网络货运量训练与测试对比图');
grid on;
%%
%使用训练好的神经网络对新输入数据进行预测
%新输入数据(2020年和2021年的相关数据)
...
disp('预测2020和2021年的公路客运量分别为(单位：万人)：');
newOutput(1,:)
disp('预测2020和2021年的公路货运量分别为(单位：万吨)：');
newOutput(2,:)
%%
%在figure(1)的基础上绘制2020和2021年的预测情况
figure(2);
t1 = 2000:2021;%注意此处范围
